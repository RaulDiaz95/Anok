...
export default function Navbar() {
  ...
  const { latest, unreadCount, refreshNotifications, markRead, markAllRead } = useNotifications();
  ...
  }, [menuOpen, notificationOpen, profileOpen]);
  ...
  const handleNotifications = async () => {
    if (!notificationOpen) {
      await refreshNotifications(true, 0, 10);
    }
  ...
  const handleOpenNotificationsPage = async () => {
    await markAllRead();
    setNotificationOpen(false);
    setMenuOpen(false);
    setProfileOpen(false);
  ...
  return (
    ...
    >
                    ...
                    {notificationOpen && (
                      ...
                      >
                        ...
                        <div className="max-h-80 overflow-auto">
                          {latest.filter((n) => !n.isRead).length === 0 && (
                            <div className="px-4 py-6 text-sm text-gray-400">No new notifications.</div>
                          )}
                          {latest.filter((n) => !n.isRead).map((notification) => (
                            <button
                              key={notification.id}
      ...
      {menuOpen && (
        <div
          ref={mobileMenuRef}
          className="md:hidden bg-[#0f0f1a]/95 backdrop-blur-lg border-t border-[#b11226]/20">
